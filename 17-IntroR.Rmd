...
Installing packages from source (downloaded from CRAN):
```{r, eval=FALSE}
...
Set eval=TRUE if tidyverse is not yet installed
```{r, eval=FALSE}
...
Assigning names can help identify what each list component contains, as well as, facilitating the extraction of values from list components.
...
`match`(a vector of values in the order you want, a vector of values to be reordered)
...
# I prefer ti use `head` -- and set the output to a limited number of columns so that it fits in the console
...
Find the hexadecimal code for your 2 favourite colors and replace the color names with the hexadecimal codes within the ggplot2 code chunk.
```{r}
...
```{r}
## Make a plot which will be written to the open device, in this case the temp file p 
...
With regular plotting you don't need the print function:
```{r}
...
**NOTE: Shortcut to write the pipe is shift + command + M on Macos**
```{r}
## A single command
sqrt(83)
...
The functional analysis that we will focus on involves gene ontology (GO) terms, which describe the roles of genes and gene products
