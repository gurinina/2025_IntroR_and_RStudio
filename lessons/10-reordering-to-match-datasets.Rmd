...
solutions(list(
  quote(match(first, second)),
  quote(second[match(first, second)])
))
...
solutions(list(
  quote(subset_rpkm <- rpkm_ordered[, -c(2, 9)])
))
...
solutions(list(
  quote(idx <- match(colnames(subset_rpkm), rownames(metadata))),
  quote(metadata_subset <- metadata[idx, ])
))
