...
solutions(list(
  quote(glengths <- c(4.6, 3000, 50000)),
  quote(species <- c("ecoli", "human", "corn")),
  quote(combined <- c(glengths, species)),
  quote(combined)
))
...
solutions(list(
  quote(samplegroup <- c("CTL", "CTL", "CTL", "KO", "KO", "KO", "OE", "OE", "OE")),
  quote(samplegroup),
  quote(samplegroup <- factor(samplegroup)),
  quote(samplegroup)
))
...
solutions(list(
  quote(titles <- c("Catch-22", "Pride and Prejudice", "Nineteen Eighty Four")),
  quote(pages <- c(453, 432, 328)),
  quote(favorite_books <- data.frame(titles, pages)),
  quote(favorite_books)
))
