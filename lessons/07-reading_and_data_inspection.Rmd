...
solutions(list(
  quote(proj_summary <- read.table("data/project-summary.txt", header = TRUE, row.names = 1)),
  quote(proj_summary)
))
```
...
solutions(list(
  quote(median(metadata$replicate))
))
```
...
solutions(list(
  quote(class(glengths)),
  quote(class(metadata)),
  quote(summary(proj_summary)),
  quote(length(samplegroup)),
  quote(dim(proj_summary)),
  quote(class(rownames(metadata))),
  quote(length(colnames(proj_summary)))
))
```
