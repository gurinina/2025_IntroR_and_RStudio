...
To install from Bioconductor, you will first need to install `BiocManager`. *This only needs to be done once ever for your R installation.* We will be installing several packages from Bioconductor using `BiocManager` (see beloow) 
...
Once you've installed these packages you should be able to run this file in R, so you can switch from the 01-getting-started.html file to the 01-getting-started.Rmd file. If want to keep using the .html file, just copy and paste the `chunk outputs` (pink bits) into your console (bottom left corner of RStudio) without the header and footer.
...
Once they are installed and the libraries for `rmarkdown`, `bookdown` and `knitr` are loaded using the library function, you can run the following command **in your console:** 
...
try to load a package and get an error, it probably means you need to install it first.
