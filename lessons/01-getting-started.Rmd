...
Download and install [XQuartz](http://xquartz.macosforge.org/) (version 2.8.5 or later)

The download link for fortran tools:

**[gfortran-12.2-universal.pkg](https://mac.r-project.org/tools/gfortran-12.2-universal.pkg)**

If you are on a Mac, you also need to install XCode. Open the terminal app (in the Finder under the GO dropdown click Utilities and in that folder click on 'Terminal.app') and then type:

sudo xcode-select --install

There is dash between xcode and select follow by a space and then a double dash before the word install which may not be visible in the pdf file. This installs Xcode command line tools which are sufficient to build R.
...
The first R command we will run is `install.packages`. R only includes a basic set of functions. It can do much more than this, and many of these functions are stored in the [CRAN](https://cran.r-project.org/). Note that these packages are vetted: they are checked for common errors and they must have a dedicated maintainer. You can easily install packages from within R if you know the name of the packages. As an example, we are going to install the three required packages.
...
Open the 01-getting started.Rmd in the lessons folder and navigate to the code below. Copy and paste the following `chunk outputs` into your console (bottom left corner of RStudio) without the header and footer backticks.
...
# COPY AND PASTE JUST THE RENDER_BOOK COMMAND INTO YOUR 
# CONSOLE AND RUN IT THERE -- ELSE IT WILL TRIGGER AN ENDLESS LOOP
# 
...
# DO NOT RUN THIS IN RMARKDOWN, COPY AND PASTE 
# publish_book() INTO YOUR CONSOLE
# AND RUN IT RUN IT THERE
