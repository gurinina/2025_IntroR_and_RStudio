...
---
# Codebook answers 69 points total

## 01 Setting up R and RStudio

```{r, include=FALSE}
knitr::opts_chunk$set(echo = TRUE) # set to FALSE to hide code
```
...
# nothing happens to number when x and y are reassigned

number

```

* Ans: nothing happens to the value of number when x and y are reassigned. 
...
* Ans: You need to rerun `number = x + y` for number to update:
...
combined # output will be in the console because of our settings
...
* Ans: the numeric values been coerced to character values.
...
Set eval=TRUE if tidyverse is not yet installed. How can you run the `BiocManager::install()` command for a package that is already installed without getting a warning message? 
```{r, eval=FALSE}
...
# typing `proj_summary` will be sent to the console as we have set the output to Chunk Output in Console (using the cogwheel to the right of the Knit arrow at the top of this window)
...
```{r}
summary(proj_summary$Intronic_Rate))
```
...
```{r}
rownames(rpkm_data)[rownames(rpkm_data) %in% important_genes]
```
...
second_reordersecond_reordered == first
```
...
Use the `match()` function to subset the metadata data frame so that the row names of the metadata data frame match the column names of the subset_rpkm data frame. Save this as a new data frame called sub_meta. 
```{r}

sub_meta <- metadata[match(colnames(subset_rpkm),rownames(metadata)),]
...
3. Add the following new layer to the code chunk theme(plot.title = element_text(hjust = 0.5)) to center the plot title. 
...
ggplot(new_metadata) +
  ...
  ggtitle("Mean expression vs Age") +
...
2. Change genotype order: Factor the `new_metadata$genotype` column without creating any extra variables/objects and change the levels to `c("Wt", "KO")`. points +1
```{r}
...
4. Change default colors points +4
...
- Ans: scale_color_manual() works if the "color" argument is used , whereas scale_fill_manual() works if the "fill" argument is used
...
- Ans: Yes. Unknown colour name: bluegreen

5. Find the hexadecimal code for your 2 favourite colors (from exercise 3 above) and replace the color names with the hexadecimal codes within the ggplot2 code chunk. points +1
...
```{r}
  ...
  theme(axis.title = element_text(size = rel(1.25))) +
  theme(plot.title=element_text(hjust = 0.5, size = rel(1.5))) +
  scale_fill_manual(values=c("#000080", "#90EE90"))
```
(End boxplot exercise)
...
# opens the graphics device for writing
...
save.image("2025_IntroR_and_RStudio.RData")
